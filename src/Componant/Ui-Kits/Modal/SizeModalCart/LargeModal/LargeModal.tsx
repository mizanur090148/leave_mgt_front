...
const LargeModal = () => {
  ...
  return (
    <>
      <Btn color="success" onClick={largeScreenToggle}>
        {LargeModals}
      </Btn>
      <CommonModal
        size="lg"
        isOpen={largeScreen}
        toggle={largeScreenToggle}
        sizeTitle="Large modal"
        modalBodyClassName="dark-modal"
      >
        <div className="large-modal-header mb-2">
          <ChevronsRight />
          <H5 className="f-w-600">{StartWithYourGoals}</H5>
        </div>
        <P className="modal-padding-space mb-0">
          No matter how talented you are as a content writer or creator, you
          will always fail if you don't have a clear set of goals.
        </P>
        <P className="modal-padding-space mb-0">
          First of all, without goals, there is no way to determine your
          success. Additionally, you lack direction.
        </P>
        <P className="modal-padding-space mb-2">
          Together with your team, respond to the following questions to make
          sure they are:
        </P>
        ...
        {largeData.map((item: any, index: number) => (
          <div className="large-modal-body mb-2" key={index}>
            <CornerUpRight />
