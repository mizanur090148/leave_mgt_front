...
import {
  bankingNextHandler,
  setNetBankingForm,
} from "../../../../../Store/Slices/VerticalWizardSlice";

const NetBankingForm = () => {
  const { netBankingFormValues } = useSelector(
    (state: any) => state.verticalWizard
  );
        ...
        <Row className="g-3">
          ...
          <Col xs="12">
            <FormGroup className="mb-0" check>
              ...
              <Label className="mb-0" for="invalidCheck67" check>
                {AgreeConditions}
              </Label>
            </FormGroup>
          </Col>
          <Col xs="12" className="text-end">
            <Btn color="success" onClick={() => dispatch(bankingNextHandler())}>
              {Finish}
            </Btn>
          </Col>
        </Row>
