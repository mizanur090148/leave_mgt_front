...
const SimilarJobsCards: React.FC<SimilarJobsCardsType> = ({
  limit,
  jobClass,
  ribbon,
  colClass,
  ...
  return (
    <Row>
      {jobData.slice(0, 5).map((item: any) => (
        <Col
          className={`col-auto ${
            limit === item.id && colClass ? "col-xl-12 xl-100" : jobClass
          }`}
          key={item.id}
        >
          <Card
            className={`${
              item.ribbon && ribbon ? "ribbon-vertical-left-wrapper" : ""
            }`}
          >
            {item.ribbon ? (
              <div
