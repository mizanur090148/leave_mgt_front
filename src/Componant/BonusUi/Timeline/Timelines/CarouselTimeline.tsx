...
const CarouselTimeline = () => {
  ...
  const next = () => {
    if (animating) return;
    const nextIndex =
      activeIndex === carouselDataList.length - 1 ? 0 : activeIndex + 1;
  ...
  const previous = () => {
    if (animating) return;
    const nextIndex =
      activeIndex === 0 ? carouselDataList.length - 1 : activeIndex - 1;
    setActiveIndex(nextIndex);
  };
  ...
  const slides = carouselDataList.map((item: any, index: number) => {
    return (
      <CarouselItem
  ...
  return (
    <VerticalTimelineElement
      ...
      iconClassName="cd-timeline-img bg-success"
    >
