...
const NonInstitutionalLiabilities = () => {
    const [isInstitutionalLiabilities, setIsInstitutionalLiabilities] = useState(false);
    const [nonInstitutionalLiabilities, setInstitutionalLiabilities] = useState<any>({});
    const [typeOfLiabilitiesEntry, setTypeOfLiabilitiesEntry] = useState("nonInstitutional");
    console.log("nonInstitutionalLiabilities", nonInstitutionalLiabilities)
    ...
    const getNonInstitutionalLiabilities = async () => {
        setLoading(true);
        await getRequest(`liability-entries/${userInfo.id}/${typeOfLiabilitiesEntry}`)
            .then((res) => {
                setLoading(false);
                setId(res?.data?.id);
                setIsInstitutionalLiabilities(res?.data ?? true);
    ...
    const {
        ...
        defaultValues: {
            type_of_liabilities_entry: nonInstitutionalLiabilities?.type_of_liabilities_entry || null,
            bank_fi_name: nonInstitutionalLiabilities?.bank_fi_name || null,
            account_no: nonInstitutionalLiabilities?.account_no || null,
            opening: nonInstitutionalLiabilities?.opening || null,
            new_loan: nonInstitutionalLiabilities?.new_loan || null,
            principal_paid:
                nonInstitutionalLiabilities?.principal_paid || null,
            invest_paid:
                nonInstitutionalLiabilities?.invest_paid || null,
            closing: nonInstitutionalLiabilities?.closing || null,
            total_principal: nonInstitutionalLiabilities?.total_principal || null,
            total_interest: nonInstitutionalLiabilities?.total_interest || null,
            name_of_person: nonInstitutionalLiabilities?.name_of_person || null,
            etin_no: nonInstitutionalLiabilities?.etin_no || null,
            name_of_loan: nonInstitutionalLiabilities?.name_of_loan || null,
            type: nonInstitutionalLiabilities?.type || null,
            total_closing: nonInstitutionalLiabilities?.total_closing || null,
            total_invest: nonInstitutionalLiabilities?.total_invest || null,
        },
    });
    ...
    useEffect(() => {
        if (id){
            console.log("nonInstitutionalLiabilities nonInstitutionalLiabilities", nonInstitutionalLiabilities);

        }
        else {
            getNonInstitutionalLiabilities();
        }
        if (nonInstitutionalLiabilities) {
            setValue("type_of_liabilities_entry", nonInstitutionalLiabilities?.type_of_liabilities_entry || null);
            setValue("bank_fi_name", nonInstitutionalLiabilities?.bank_fi_name || "");
            setValue("account_no", nonInstitutionalLiabilities?.account_no || "");
            setValue("opening", nonInstitutionalLiabilities?.opening || null);
            setValue("new_loan", nonInstitutionalLiabilities?.new_loan || null);
            setValue("principal_paid", nonInstitutionalLiabilities?.principal_paid || null);
            setValue("invest_paid", nonInstitutionalLiabilities?.invest_paid || null);
            setValue("closing", nonInstitutionalLiabilities?.closing || null);
            setValue("total_principal", nonInstitutionalLiabilities?.total_principal || null);
            setValue("total_interest", nonInstitutionalLiabilities?.total_interest || null);
            setValue("name_of_person", nonInstitutionalLiabilities?.name_of_person || "");
            setValue("etin_no", nonInstitutionalLiabilities?.etin_no || "");
            setValue("name_of_loan", nonInstitutionalLiabilities?.name_of_loan || "");
            setValue("type", nonInstitutionalLiabilities?.type || null);
            setValue("total_closing", nonInstitutionalLiabilities?.total_closing || null);
            setValue("total_invest", nonInstitutionalLiabilities?.total_invest || null);
        }
    }, [nonInstitutionalLiabilities, setValue]);
                            ...
                            <Col sm="6" md="4">
                                <FormGroup>
                                    ...
                                    <Input
                                        ...
                                        className={`${errors.name_of_person ? "is-invalid" : ""}`}
                                        {...register("name_of_person")}
                                        defaultValue={name_of_person}
                                        onChange={(e) =>
                                            setValue("name_of_person", parseInt(e.target.value))
                                        }
                                    />
                                </FormGroup>
                            </Col>
                            <Col sm="6" md="4">
                                <FormGroup>
                                    ...
                                    <Input
                                        ...
                                        {...register("etin_no")}
                                        defaultValue={etin_no}
                                        onChange={(e) =>
                                            setValue("etin_no", parseInt(e.target.value))
                                        }
                                    />
                                </FormGroup>
                            </Col>
