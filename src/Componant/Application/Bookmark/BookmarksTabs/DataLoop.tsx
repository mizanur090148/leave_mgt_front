...
const DataLoop = () => {
  const { bookmark } = useSelector((state: any) => state.bookmarkTab);
  const dispatch = useDispatch();

  const addToFavourites = (data: BookMarkData) => {
    if (data.fillStar === false) {
      const newBookMarkData = bookmark.map((item: any) =>
        item.id === data.id ? { ...item, fillStar: true } : item
      );
      dispatch(setBookmark(newBookMarkData));
    } else {
      const newBookMarkData = bookmark.map((item: any) =>
        item.id === data.id ? { ...item, fillStar: false } : item
      );
      dispatch(setBookmark(newBookMarkData));
    }
  };
  ...
  return (
    <Row id="bookmarkData">
      {bookmark.length > 0 ? (
        bookmark.map((data: any, index: any) => (
          <Col
            xxl="3"
            xl="4"
            md="4"
