...
const AgricultureLand: React.FC = () => {
  ...
  const getItems = async () => {
    try {
      ...
      const res = await getRequest(
        `agri-non-agri-lands?type=agri&past_return=1`
      );
  ...
  const updateHeadData = (updatedData: any) => {
    const result = updatedData?.reduce(
      (acc: any, curr: any) => acc + Number(curr.net_value_of_property || 0),
      0
    );
    const updatedState = {
      ...pastReturnData,
      agriLand: result,
      total: pastReturnTotalData({ ...pastReturnData, agriLand: result })
    };
  ...
  return (
    <Col xl="9">
      ...
      <Card className="profile-right">
        <CardHeaderCommon title="Agriculture Land" tagClass="card-title mb-0" />
        <CardBody>
          {loading ? (
