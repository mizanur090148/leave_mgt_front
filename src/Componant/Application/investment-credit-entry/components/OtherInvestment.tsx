...
const OtherInvestment = () => {
    ...
    const getData = async () => {
        setLoading(true);
        await getRequest(`other-investments`)
            .then((res: any) => {
                setLoading(false);
                const data = res.data?.map((item: any) => ({
                    id: item.id,
                    type: item.type,
                    name_of_zakat_fund: item.name_of_zakat_fund,
                    account_no: item.account_no,
                    name_of_fund: item.name_of_fund,
                    ref_no: item.ref_no,
    ...
    const onSubmit: SubmitHandler<{ items: PropertyItem[] }> = async (data) => {
        ...
        try {
            const res = await postRequest(`other-investments`, inputArr);
            setOpen(true);
    ...
    const addMore = (type: string) => {
        ...
        setValue(`items.${newIndex}.name_of_zakat_fund`, "");
        setValue(`items.${newIndex}.account_no`, "");
        setValue(`items.${newIndex}.name_of_fund`, "");
        setValue(`items.${newIndex}.ref_no`, "");
    ...
    const deleteAction = async (id: any, type: string, index: any) => {
        ...
        if (confirmation) {
            ...
            if (id) {
                await deleteRequest(`other-investments/${id}`)
                    .then((res) => {
    ...
    const getFieldLabel = (type: string) => {
        switch (type) {
            case "zakat":
                return {
                    firstLabel: "Name Of Zakat Fund",
                    secondLabel: "Account No",
                    thirdLabel: "Contribution Amount",
                };
            case "nbr":
                return {
                    firstLabel: "Name Of Fund",
                    secondLabel: "Ref. No",
                    thirdLabel: "Contribution Amount",
                };
    ...
    if (watch("items")) {
        const zakatData = dataItems?.filter((item) => item.type === "zakat");
        totalZakat = zakatData?.reduce((n, {contribution_amount}) => Number(n) + Number(contribution_amount), 0)

        const nbrData = dataItems?.filter((item) => item.type === "nbr");
        totalNbr = nbrData?.reduce((n, {contribution_amount}) => Number(n) + Number(contribution_amount), 0)
                                                            ...
                                                            <Col>
                                                                <FormGroup>
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        placeholder=""
                                                                        {...register(
                                                                            `items.${item.index}.name_of_zakat_fund`,
                                                                            {
                                                                        ...
                                                                        defaultValue={item.name_of_zakat_fund}
                                                                    />
                                                            ...
                                                            <Col>
                                                                <FormGroup>
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        defaultValue={item.name_of_fund}
                                                                    />
