...
const RightChatBody = () => {
  ...
  const { allMembers, chats, selectedUser, currentUser } = useSelector(
  ...
  const fetchChatAsync = () => {
    if (chats.length > 0) {
      ...
      const chat = chats.filter((x: ChatProp) =>
        x.users.includes(currentUserId)
      );
      const selectedUser = chats[0].users.find(
        (x: number) => x !== currentUserId
      );
      const oneSelect = allMembers.find(
        (x: ChatUserProfiles) => x.id === selectedUser
      );
      ...
      if (allMembers.length > 0)
        allMembers.find((x: ChatUserProfiles) => x.id === selectedUser);
    }
  };
          ...
          {selectedChat && selectedChat.messages.length > 0 ? (
            selectedChat.messages.map((item: MessageType, id: number) => {
              const participators = allMembers.find(
                (x: ChatUserProfiles) => x.id === item.sender
              );
              return (
                <div
                  className={`msg ${
                ...
                >
                  {item?.name ? (
                    ...
                    <Image
                      src={dynamicImage(participators?.image)}
