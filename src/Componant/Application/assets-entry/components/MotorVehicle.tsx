...
import { assetEntry, pastReturn } from "../../../../Store/Slices/PastReturnTotalSlice";
...
const MotorVehicle: React.FC = () => {
  ...
  const getItems = async () => {
    try {
      setLoading(true);
      const res = await getRequest(`motor-vehicles`);
      const data = res.data?.map((item: any) => ({
        ...
        closing_cost: item?.closing_cost,
  ...
  const updateHeadData = (updatedData: any) => {
    const result = updatedData?.reduce(
      (acc: any, curr: any) => acc + Number(curr.closing_cost || 0),
      0
    );
    const updatedState = {
      ...assetEntryData,
      motorVehicle: result,
      total: assetEntryTotalData({ ...assetEntryData, motorVehicle: result })
    };
                        ...
                        <FormGroup>
                          ...
                          {item?.past_return ? (
                            ...
                            >
                              {vehicleTypes
                                ?.filter(
                                  (type) => type.id === items[key].type_id
                                )
                                .map((data: any, index) => (
                        ...
                        <FormGroup>
                          ...
                          {item?.past_return ? (
                            ...
                            >
                              {capacities
                                ?.filter(
                                  (type) => type.id === items[key].capacity_id
                                )
                                .map((data: any, index) => (
                                  <option key={index} value={data.id}>
                                    {data.name}
                        ...
                        <FormGroup>
                          ...
                          {item?.past_return ? (
                            <Input
                              type="text"
                              disabled
                              defaultValue={item.cost_with_registration}
                            />
                          ) : (
                            <Fragment>
                              <Input
                                type="number"
                                placeholder="Opening Cost"
                                {...register(
                      ...
                      <Col sm="4" md="4">
                        <FormGroup>
                          ...
                          <Input
                            ...
                            {...register(`items.${key}.cost_of_sale`, {
                              required: "This is required",
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].cost_of_sale = value;
                                setItems(updatedLands);
                                setValue(`items.${key}.cost_of_sale`, value);
                              },
                            })}
                            defaultValue={item.cost_of_sale}
                          />
                          {errors.items?.[key]?.cost_of_sale && (
                            <span className="error-msg">
                              {errors.items[key]?.cost_of_sale?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
                      <Col sm="4" md="4">
                        <FormGroup>
                          ...
                          <Input
                            ...
                            {...register(`items.${key}.closing_cost`, {
                              required: "This is required",
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].closing_cost = value;
                                setItems(updatedLands);
                                setValue(`items.${key}.closing_cost`, value);
                              },
                            })}
                            defaultValue={item.closing_cost}
                          />
                          {errors.items?.[key]?.closing_cost && (
                            <span className="error-msg">
                              {errors.items[key]?.closing_cost?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
