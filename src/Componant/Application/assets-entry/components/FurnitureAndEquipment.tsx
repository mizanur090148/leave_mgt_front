...
interface PropertyItem {
  ...
  sale_disposal_qty: number;
  sale_disposal_value: number;
  past_return?: boolean;
}
...
const FurnitureAndEquipment: React.FC = () => {
  ...
  const getItems = async () => {
    setLoading(true);
    try {
      const res = await getRequest(`furniture-equipments`);
      const data = res.data?.map((item: any) => ({
        ...
        sale_disposal_qty: item.sale_disposal_qty,
        sale_disposal_value: item.sale_disposal_value,
  ...
  const updateHeadData = (updatedData: any) => {
    const result = updatedData?.reduce(
      (acc: any, curr: any) =>
        ...
        Number(curr.sale_disposal_value || 0),
      0
    );
    const updatedState = {
      ...assetEntryData,
      furniture: result,
      total: assetEntryTotalData({ ...assetEntryData, furniture: result })
    };
                ...
                {items.map((item, key) => (
                  <Fragment key={key}>
                    ...
                    <Row>
                      ...
                      <Col sm="4" md="4">
                        <FormGroup>
                          <Label>Sale/Disposal Qty</Label>
                          <Input
                            type="text"
                            placeholder="Sale/Disposal Qty"
                            {...register(`items.${key}.sale_disposal_qty`, {
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].sale_disposal_qty = value;
                                setItems(updatedLands);
                                setValue(
                                  `items.${key}.sale_disposal_qty`,
                                  value
                                );
                              },
                            })}
                            defaultValue={item.sale_disposal_qty}
                          />
                          {errors.items?.[key]?.sale_disposal_qty && (
                            <span className="error-msg">
                              {errors.items[key]?.sale_disposal_qty?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
                      <Col sm="4" md="4">
                        <FormGroup>
                          ...
                          <Input
                            type="text"
                            placeholder="Sale/Disposal Value"
                            {...register(`items.${key}.sale_disposal_value`, {
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].sale_disposal_value = value;
                                setItems(updatedLands);
                                setValue(
                                  `items.${key}.sale_disposal_value`,
                                  value
                                );
                              },
                            })}
                            defaultValue={item.sale_disposal_value}
                          />
                          {errors.items?.[key]?.sale_disposal_value && (
                            <span className="error-msg">
                              {errors.items[key]?.sale_disposal_value?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
                        ...
                        <FormGroup>
                          ...
                          <Input
                            ...
                            value={
                              ...
                              Number(item.sale_disposal_qty || 0)
                            }
                            disabled
                          />
                        </FormGroup>
                        ...
                        <FormGroup>
                          ...
                          <Input
                            ...
                            value={
                              ...
                              Number(item.sale_disposal_value || 0)
                            }
                            disabled
                          />
                        </FormGroup>
