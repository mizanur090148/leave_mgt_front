...
const InstitutionalLiabilities: React.FC = () => {
  ...
  const getchitems = async () => {
    setLoading(true);
    try {
      const res = await getRequest(`institutional-liabilities`);
      const data = res.data?.map((item: any) => ({
        id: item.id,
        type_of_liabilities_entry: item.type_of_liabilities_entry,
        bank_fi_name: item.bank_fi_name,
  ...
  const deleteAction = async (id: any, index: number) => {
    ...
    if (confirmation) {
      ...
      if (id) {
        await deleteRequest(`institutional-liabilities/${id}`)
          .then((res) => {
  ...
  const onSubmit: SubmitHandler<{ items: PropertyItem[] }> = async (data) => {
    ...
    await postRequest(`institutional-liabilities`, inputArr)
      .then((res) => {
        setOpen(true);
  ...
  const updateHeadData = (updatedData: any) => {
    ...
    const updatedState = {
      ...
      total: liabilitiesTotalData({ ...liabilityData, institutional: result })
    };
  ...
  return (
    <Col xl="9">
      <LiabilityTop title="Institutional Liabilities" itemName="institutional"  />
                      ...
                      <Col sm="4" md="4">
                        <FormGroup>
                          ...
                          <Input
                            ...
                            {...register(`items.${key}.bank_fi_name`, {
                              required: "This is required",
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].bank_fi_name = value;
                                setItems(updatedLands);
                                setValue(`items.${key}.bank_fi_name`, value);
                              },
                            })}
                            defaultValue={item.bank_fi_name}
                          />
                          {errors.items?.[key]?.bank_fi_name && (
                            <span className="error-msg">
                              {errors.items[key]?.bank_fi_name?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
                      <Col sm="4" md="4">
                        <FormGroup>
                          ...
                          <Input
                            type="text"
                            placeholder="A/C No"
                            {...register(`items.${key}.account_no`, {
                              onChange: (e) => {
                                const value = e.target.value;
                                const updatedLands = [...items];
                                updatedLands[key].account_no = value;
                                setItems(updatedLands);
                                setValue(`items.${key}.account_no`, value);
                              },
                            })}
                            defaultValue={item.account_no}
                          />
                          {errors.items?.[key]?.account_no && (
                            <span className="error-msg">
                              {errors.items[key]?.account_no?.message}
                            </span>
                          )}
                        </FormGroup>
                      </Col>
