...
const OtherLiabilities: React.FC = () => {
    ...
    const getchitems = async () => {
        setLoading(true);
        try {
            const res = await getRequest(`other-liabilities`);
            const data = res.data?.map((item: any) => ({
                id: item.id,
                type_of_liabilities_entry: item.type_of_liabilities_entry,
                name_of_loan: item.name_of_loan,
                type: item.type,
    ...
    const deleteAction = async (id: any, index: number) => {
        ...
        if (confirmation) {
            ...
            if (id) {
                await deleteRequest(`other-liabilities/${id}`)
                    .then((res) => {
    ...
    const onSubmit: SubmitHandler<{ items: PropertyItem[] }> = async (data) => {
        ...
        await postRequest(`other-liabilities`, inputArr)
            .then((res) => {
                setOpen(true);
    ...
    const updateHeadData = (updatedData: any) => {
        ...
        const updatedState = {
            ...liabilityData,
            otherLiabilities: result,
            total: liabilitiesTotalData({ ...liabilityData, otherLiabilities: result })
        };
    ...
    return (
        <Col xl="9">
            <LiabilityTop title="Other Liabilities" itemName="otherLiabilities" />
            <Card className="profile-right">
                <CardHeaderCommon title="Other Liabilities" tagClass="card-title mb-0" />
                <CardBody>
                    {loading ? (
                                            ...
                                            <Col sm="4" md="4">
                                                <FormGroup>
                                                    ...
                                                    <Input
                                                        ...
                                                        {...register(`items.${key}.name_of_loan`, {
                                                            required: "This is required",
                                                            onChange: (e) => {
                                                                const value = e.target.value;
                                                                const updatedLands = [...items];
                                                                updatedLands[key].name_of_loan = value;
                                                                setItems(updatedLands);
                                                                setValue(`items.${key}.name_of_loan`, value);
                                                            },
                                                        })}
                                                        defaultValue={item.name_of_loan}
                                                    />
                                                    {errors.items?.[key]?.name_of_loan && (
                                                        <span className="error-msg">
                                                            {errors.items[key]?.name_of_loan?.message}
                                                        </span>
                                                    )}
                                                </FormGroup>
                                            </Col>
